-- ================================================================
-- DEV-CAT-01.6
-- Decouple operational inventory consumption from option pricing
--
-- inventory_items.consumption_quantity becomes the canonical
-- source for the quantity consumed by a product option.
--
-- product_option_prices remains responsible only for commercial
-- pricing. sale_item_options.quantity preserves the operational
-- snapshot used by process_sale_inventory.
-- ================================================================

CREATE OR REPLACE FUNCTION public.create_local_sale_with_order_v2(
    ...
    p_reward_id bigint DEFAULT NULL::bigint,
...
declare
    ...
    -- Promoción automática Shopify.
    -- El Premio Misterioso no corresponde a una línea gift:
    -- es un componente comercial del descuento calculado
    -- sobre los potes físicos no-regalo del pedido.
    v_mystery_reward_discount_total integer := 0;
    v_mystery_reward_unit_price integer := 0;
    v_pot_discount_base integer := 0;

    -- Reward aplicado a la venta.
    v_reward public.customer_rewards%rowtype;
    v_reward_product_id bigint;
    v_reward_discount_total integer := 0;
    v_reward_applied boolean := false;
    v_reward_applied_to_line boolean := false;
    v_reward_sale_item_id bigint;
    v_redeemed_at timestamptz;

    -- Se conserva para compatibilidad/auditoría del resultado.
    v_custom_payable_subtotal integer := 0;

    -- Suma de total_price de todas las lí­neas cuyo snapshot
    ...
    -- Snapshot definitivo para la lí­nea actual.
...
begin
    ...
    then
        raise exception
            'La venta debe tener al menos un í­tem.';
    end if;
    ...
    if not exists (
        ...
        where sc.code = v_channel
          and sc.is_active = true
    ) then
        raise exception
            'Canal de venta inexistente o inactivo: %',
            v_channel;
    end if;
    ...
    then
        raise exception
            'Los pedidos digitales requieren un número externo.';
    end if;
    ...
    if p_reward_id is not null then
        ...
        if p_customer_id is null then
            raise exception
                'El canje de premio requiere un cliente.';
        end if;


        -- El cliente debe seguir siendo elegible al momento
        -- efectivo de confirmar el canje.
        if not exists (
            select 1
            from public.clientes c
            where c.id = p_customer_id
              and c.email_verificado = true
              and c.tarjeta_activa = true
        ) then
            raise exception
                'El cliente no se encuentra habilitado para canjear premios.';
        end if;
        ...
        if not found then
            raise exception
                'El premio indicado no existe o no pertenece al cliente.';
        end if;


        if v_reward.status <> 'active' then
            raise exception
                'El premio seleccionado no se encuentra activo.';
        end if;


        if v_reward.expires_at is not null
           and v_reward.expires_at < now()
        then
            raise exception
                'El premio seleccionado está vencido.';
        end if;


        if v_reward.reward_product_id is null then
            raise exception
                'El premio seleccionado no tiene un producto asociado.';
        end if;


        v_reward_product_id :=
            v_reward.reward_product_id;


        if not exists (
            select 1
            from public.products
            where id = v_reward_product_id
              and is_active = true
        ) then
            raise exception
                'El producto asociado al premio no existe o está inactivo.';
        end if;
    ...
    if v_manual_discount_type is null then
        ...
        if v_manual_discount_type not in ('percent', 'fixed') then
            raise exception
                'Tipo de descuento manual inválido.';
        end if;
        ...
        if v_manual_discount_reason not in (
            ...
            raise exception
                'Motivo de descuento manual inválido.';
        end if;
    ...
    loop
        ...
        if v_item_type = 'CUSTOM' then
            ...
            if v_custom_name is null then
                raise exception
                    'El í­tem personalizado requiere un nombre.';
            end if;


            if v_custom_unit_price <= 0 then
                raise exception
                    'El í­tem personalizado requiere un precio mayor que cero.';
            end if;
            ...
            then
                raise exception
                    'Los í­tems personalizados no pueden registrarse como regalo.';
            end if;
            ...
            then
                raise exception
                    'Los í­tems personalizados no pueden incluir opciones de catálogo.';
            end if;


            -- --------------------------------------------------------
            -- P0.04
            -- CUSTOM puede declarar explí­citamente elegibilidad.
            --
            ...
            if v_item ? 'loyalty_eligible' then
                ...
                if jsonb_typeof(v_item->'loyalty_eligible') <> 'boolean' then
                    raise exception
                        'La elegibilidad de fidelización del í­tem personalizado % debe ser booleana.',
                        v_custom_name;
                end if;
        ...
        if not exists (
            select 1
            from public.product_channels pc
            where pc.product_id = v_product.id
              and pc.channel_code = v_channel
              and pc.is_enabled = true
        ) then
            raise exception
                'El producto % no está habilitado para el canal %.',
                v_product.name,
                v_channel;
        end if;
        ...
        -- ------------------------------------------------------------
        -- INC-LOY-02
        -- Todo PRODUCT de catálogo aporta monetariamente al acumulado
        -- diario de fidelización.
        --
        -- La elegibilidad explícita Sí/No se mantiene exclusivamente
        -- para líneas CUSTOM.
        -- ------------------------------------------------------------

        v_item_loyalty_eligible := true;
        ...
        then
            ...
            -- Precio comercial
            --
            -- El precio sigue siendo responsabilidad de
            ...
            -- Consumo operacional
            --
            -- La cantidad consumida pertenece al inventory_item
            -- asociado a Producto + Opción y es independiente
            -- del precio y del canal comercial.
            -- --------------------------------------------------------

            select
                ii.consumption_quantity
            ...
            if v_coffee_option_price is null then
                raise exception
                    'El tipo de café seleccionado no tiene un precio activo para el canal %.',
                    v_channel;
            end if;
            ...
            then
                raise exception
                    'El tipo de café seleccionado no tiene un consumo de inventario válido.';
            end if;
        ...
        -- ============================================================
        --
        -- CAFE es un producto option-priced:
        -- su precio comercial completo proviene de
        -- product_option_prices para el canal seleccionado.
        --
        -- Los demás productos obtienen su precio base desde
        -- product_prices.
        -- ============================================================
        ...
        then
            ...
            if v_price is null then
                raise exception
                    'Producto sin precio activo para el canal %: %',
                    v_channel,
                    v_product.name;
            end if;
        ...
        --
        -- Gift:
        --   descuenta la línea completa.
        --
        -- Reward:
        --   cubre exactamente 1 unidad del producto asociado.
        --   Nunca se aplica sobre una línea marcada como gift.
        -- ------------------------------------------------------------

        v_reward_applied_to_line := false;
        ...
        elsif p_reward_id is not null
              and not v_reward_applied
              and v_product.id = v_reward_product_id
        then

            v_line_discount :=
                v_unit_price;

            v_reward_discount_total :=
                v_reward_discount_total
                + v_unit_price;

            v_reward_applied := true;
            v_reward_applied_to_line := true;
        ...
        if v_reward_applied_to_line then

            v_reward_sale_item_id :=
                v_sale_item_id;
        ...
        -- El snapshot controla la contribución monetaria.
        ...
        then
            ...
            v_pot_subtotal :=
                ...
                + v_gross_line_total
                - case
                    when v_reward_applied_to_line
                        then v_line_discount
                    else 0
                  end;


            -- Sólo esta fracción puede reducir el monto loyalty.
            if v_item_loyalty_eligible then
        ...
        then
            ...
            if not found then
                raise exception
                    'No fue posible registrar el tipo de café seleccionado.';
            end if;
    ...
    -- ================================================================
    -- 10. DESCUENTO POT / PROMOCIÓN SHOPIFY
    -- ================================================================

    -- Shopify nunca debe generar un pedido de exactamente 6 potes
    -- físicos no-regalo: al comprar 6, incorpora un séptimo
    -- POT-16-LISTO como Premio Misterioso.
    if v_channel = 'shopify'
       and v_pot_quantity = 6
    then
        raise exception
            'Un pedido Shopify de 6 potes no es válido: debe incluir el Premio Misterioso como séptimo pote.';
    end if;


    -- El Premio Misterioso no corresponde a una línea gift.
    -- Desde 7 potes físicos Shopify, se descuenta comercialmente
    -- el valor vigente de un POT-16-LISTO en Shopify.
    if v_channel = 'shopify'
       and v_pot_quantity >= 7
    then

        select
            pp.price
        into
            v_mystery_reward_unit_price
        from public.product_prices pp
        join public.products p
          on p.id = pp.product_id
         and p.is_active = true
        join public.product_channels pc
          on pc.product_id = p.id
         and pc.channel_code = 'shopify'
         and pc.is_enabled = true
        where p.sku = 'POT-16-LISTO'
          and pp.channel = 'shopify'
          and pp.price_list = 'general'
        ...
        if v_mystery_reward_unit_price is null
           or v_mystery_reward_unit_price <= 0
        then
            raise exception
                'No existe un precio Shopify activo para POT-16-LISTO requerido por la promoción Premio Misterioso.';
        end if;


        v_mystery_reward_discount_total :=
            v_mystery_reward_unit_price;

    else

        v_mystery_reward_discount_total := 0;

    end if;


    -- La base porcentual excluye el valor equivalente al Premio
    -- Misterioso. Los regalos normales de Nook ya fueron excluidos
    -- del conteo y del subtotal promocional durante el procesamiento
    -- de líneas.
    v_pot_discount_base :=
        greatest(
            0,
    ...
    v_pot_discount_rate :=
        case
            ...
            when v_channel = 'local'
                 and v_pot_quantity >= 4
                then 0.10

            -- Shopify:
            -- 4-5 físicos = 10%
            -- 6 físicos = inválido
            -- 7-8 físicos = 10% + Premio Misterioso
            -- 9+ físicos = 15% + Premio Misterioso
            when v_channel = 'shopify'
                 and v_pot_quantity >= 9
                then 0.15
            ...
            -- Rappi / Uber Eats / otros canales:
            -- sin descuento automático por volumen.
            else 0
        end;
    ...
    --
    -- El Premio Misterioso se descuenta por separado del monto
    -- elegible loyalty. Por eso se excluye su equivalente de la
    -- base antes de aplicar el descuento porcentual de potes.
    ...
    -- ================================================================
    -- 11. TOTAL ANTES DE DESCUENTO MANUAL
    -- ================================================================
    ...
    -- ================================================================
    -- 15. DESCUENTO MANUAL SOBRE MONTO ELEGIBLE
    -- ================================================================
    ...
    -- ================================================================
    -- 16. GUARDRAILS PATRIMONIALES
    -- ================================================================

    if v_loyalty_eligible_total < 0 then
        raise exception
            'El monto elegible de fidelización no puede ser negativo.';
    end if;


    if v_loyalty_eligible_total > v_total then
        raise exception
            'El monto elegible de fidelización no puede superar el total de la venta.';
    end if;
    ...
    if exists (
        ...
        raise exception
            'Una o más lí­neas quedaron sin snapshot de elegibilidad.';
    end if;


    -- ================================================================
    -- 17. ACTUALIZAR CABECERA
    -- ================================================================
    ...
    -- ================================================================
    -- 20. CANJE TRANSACCIONAL DE REWARD
    -- ================================================================

    if p_reward_id is not null then

        -- El reward fue validado al inicio, pero además debe haberse
        -- materializado efectivamente sobre una línea de la venta.
        if not v_reward_applied
           or v_reward_sale_item_id is null
        then
            raise exception
                'El premio seleccionado no pudo aplicarse a la venta.';
        end if;
        ...
        if not found then
            raise exception
                'El premio seleccionado ya no se encuentra disponible.';
        end if;


        -- Mantener compatibilidad con campañas históricas/futuras
        -- que utilicen customer_rewards.
        if v_reward.campaign_id is not null then

            update public.campana_clientes
            set
                estado = 'canjeado',
                canjeado_at = v_redeemed_at
            where cliente_id = p_customer_id
              and campana_id = v_reward.campaign_id
              and (
                    premio_id = v_reward.legacy_reward_id
                    or premio_id = v_reward.id
              );
        ...
        perform public.record_customer_event(
            ...
            p_event_type := 'loyalty.reward_redeemed',
            p_source_module := 'loyalty',
            p_source_entity_type := 'customer_reward',
            p_source_entity_id := p_reward_id::text,
            p_sale_id := v_sale_id,
            ...
            p_actor_identifier := nullif(
                trim(coalesce(p_actor_identifier, '')),
                ''
            ),
            p_occurred_at := v_redeemed_at,
            p_idempotency_key :=
                'loyalty-reward-redeemed:' || p_reward_id,
            p_metadata := jsonb_build_object(
                'rewardName', v_reward.name,
                'rewardType', v_reward.reward_type,
                'rewardProductId', v_reward_product_id,
                'saleItemId', v_reward_sale_item_id,
                'rewardDiscountTotal', v_reward_discount_total,
                'campaignId', v_reward.campaign_id,
                'legacyRewardId', v_reward.legacy_reward_id,
        ...
        set fecha_ultimo_canje =
            case
                when fecha_ultimo_canje is null
                    then v_redeemed_at
                else greatest(
                    fecha_ultimo_canje,
                    v_redeemed_at
...
-- PERMISOS
-- ================================================================
-- La RPC de creación de venta es exclusivamente server-side.
-- Se preserva el modelo productivo previo:
-- postgres + service_role con EXECUTE.
-- ================================================================

REVOKE ALL
ON FUNCTION public.create_local_sale_with_order_v2(
    bigint,
    text,
    jsonb,
    bigint,
