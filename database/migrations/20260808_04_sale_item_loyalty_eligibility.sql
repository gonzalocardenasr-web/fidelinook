-- =====================================================================
-- NOOK
-- P0.04 — SALE ITEM LOYALTY ELIGIBILITY SNAPSHOT
-- 2026-08-08
--
-- OBJETIVO
-- ---------------------------------------------------------------------
-- Blindar la construcción de sales.loyalty_eligible_total.
--
-- PRINCIPIOS
-- ---------------------------------------------------------------------
-- 1. Cada sale_item conserva un snapshot histórico de elegibilidad:
--
--      sale_items.loyalty_eligible
--
-- 2. PRODUCT:
--      la elegibilidad se obtiene server-side desde
--      products.generates_stamps.
--
--      El frontend NO decide este valor.
--
-- 3. CUSTOM:
--      puede declarar loyalty_eligible explícitamente.
--
--      Si un caller antiguo no envía el atributo, se interpreta
--      conservadoramente como FALSE.
--
-- 4. sales.loyalty_eligible_total:
--      se calcula desde las líneas elegibles de LA VENTA,
--      no desde el catálogo vivo posteriormente.
--
-- 5. Las ventas históricas NO se reinterpretan utilizando el catálogo
--    actual:
--
--      PRODUCT histórico -> TRUE
--      CUSTOM histórico  -> FALSE
--
--    Esto preserva exactamente la regla utilizada hasta P0.04:
--      - PRODUCT pagado participaba en loyalty.
--      - CUSTOM no participaba.
--
-- 6. gifts:
--      mantienen loyalty_eligible como snapshot del producto,
--      pero aportan $0 porque total_price = 0.
--
-- 7. descuentos:
--      - descuento de potes reduce sólo el monto elegible de potes
--        que sean loyalty_eligible.
--      - descuento manual porcentual se aplica proporcionalmente
--        sobre el monto elegible.
--      - descuento manual fijo reduce el monto elegible hasta agotarlo.
--
-- 8. products.stamp_quantity NO participa en este DEV.
--    La conversión monto -> sellos sigue siendo responsabilidad de
--    Daily Loyalty.
-- =====================================================================


-- =====================================================================
-- 1. SNAPSHOT HISTÓRICO EN sale_items
-- =====================================================================

ALTER TABLE public.sale_items
ADD COLUMN IF NOT EXISTS loyalty_eligible boolean;
...
--
-- IMPORTANTE:
-- NO usamos products.generates_stamps para reconstruir historia.
--
-- El sistema anterior trataba:
--   PRODUCT = elegible
--   CUSTOM  = no elegible
--
-- Preservamos ese significado histórico.
-- ---------------------------------------------------------------------

UPDATE public.sale_items
SET loyalty_eligible =
  CASE
    WHEN upper(trim(coalesce(item_type, 'PRODUCT'))) = 'CUSTOM'
      THEN false
    ELSE true
  END
...
ALTER TABLE public.sale_items
ALTER COLUMN loyalty_eligible SET NOT NULL;


COMMENT ON COLUMN public.sale_items.loyalty_eligible IS
'Snapshot de elegibilidad monetaria para fidelización al momento de la venta. PRODUCT deriva de products.generates_stamps; CUSTOM se declara explícitamente.';


-- =====================================================================
-- 2. REEMPLAZAR create_local_sale_with_order
-- =====================================================================
...
declare
    ...
    -- Suma de total_price de todas las líneas cuyo snapshot
    ...
    -- Snapshot definitivo para la línea actual.
...
begin
    ...
    loop
        ...
        if v_item_type = 'CUSTOM' then
            ...
            if v_custom_name is null then
                raise exception
                    'El ítem personalizado requiere un nombre.';
            end if;


            if v_custom_unit_price <= 0 then
                raise exception
                    'El ítem personalizado requiere un precio mayor que cero.';
            end if;
            ...
            -- --------------------------------------------------------
            -- P0.04
            -- CUSTOM puede declarar explícitamente elegibilidad.
            --
            ...
            if v_item ? 'loyalty_eligible' then
                ...
                if jsonb_typeof(v_item->'loyalty_eligible') <> 'boolean' then
                    raise exception
                        'La elegibilidad de fidelización del ítem personalizado % debe ser booleana.',
                        v_custom_name;
                end if;
        ...
        --
        -- Siempre se toma directamente del catálogo server-side.
        -- ------------------------------------------------------------
        ...
        then
            ...
            if jsonb_typeof(v_item->'options') <> 'array' then
                raise exception
                    'El producto Café requiere seleccionar exactamente un tipo de café.';
            end if;
            ...
            then
                raise exception
                    'El producto Café requiere seleccionar exactamente un tipo de café válido.';
            end if;
            ...
            if v_coffee_option_price is null then
                raise exception
                    'El tipo de café seleccionado no tiene un precio activo.';
            end if;
        ...
        then
            raise exception
                'El producto % está marcado como regalo y requiere motivo.',
                v_product.name;
        end if;
    ...
    -- Todas las líneas deben haber quedado snapshotteadas.
    if exists (
        select 1
        ...
        raise exception
            'Una o más líneas quedaron sin snapshot de elegibilidad.';
    end if;
